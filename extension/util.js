...
const getWorkspace = (context) => {
  ...
  if (context && !vscode.workspace && !vscode.workspace.workspaceFolders) {
    ...
    if (
      ...
      try {
        ...
        logger(
          localize("debug.logger.error", "getWorkspace:fetch", err.toString()),
          "error"
        );
      }
    }
  ...
  if (false) {
    ...
    vscode.window.showErrorMessage(
      `${localize("extension.title")}: ${message}`
    );
  }
...
const parseFilePath = (_file, _root = "") => {
  return _await(this, void 0, void 0, function* () {
    ...
    try {
      ...
      logger(
        localize("debug.logger.error", "parseFilePath", err.toString()),
        "error"
      );
...
const updateConfig = (excludes, callback, message) => {
  try {
    ...
    logger(
      localize("debug.logger.error", "updateConfig", err.toString()),
      "error"
    );
...
function exclude(uri, callback) {
  return _await(this, void 0, void 0, function* () {
    try {
      ...
      let _showPicker = vscode.workspace
        .getConfiguration()
        .get(
      ...
      if (selections && selections.length > 0) {
        ...
        try {
          ...
          logger(
            localize("debug.logger.error", "exclude:update", err.toString()),
            "error"
          );
          vscode.window.showErrorMessage(err.message || err);
        }
      ...
      logger(
        localize("debug.logger.error", "exclude", err.toString()),
        "error"
      );
      vscode.window.showErrorMessage(err.message || err);
    }
...
function getResourcePath(file, theme) {
  return theme
    ? context.asAbsolutePath(path.join("extension", "resources", theme, file))
    : context.asAbsolutePath(path.join("extension", "resources", file));
}
...
function toggleAll(callback) {
  try {
    const excludes = vscode.workspace
      .getConfiguration()
      .get("files.exclude", vscode.ConfigurationTarget.Workspace);
    const backup = vscode.workspace
      .getConfiguration()
      .get("explorerExclude.backup", vscode.ConfigurationTarget.Workspace);
    ...
    logger(
      localize("debug.logger.error", "toggleAll", err.toString()),
      "error"
    );
    vscode.window.showErrorMessage(err.message || err);
  }
